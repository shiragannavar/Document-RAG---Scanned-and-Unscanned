...
ANSWER_PROMPT = ChatPromptTemplate.from_template(
    """You are an expert legal assistant for question-answering tasks. Use only the following context to answer the question. If you don't know the answer, just say that you don't know. Be as verbose in your response as possible.The answer should be in JSON format with answer and references keys.
    ...
    for page_text, page_num in page_texts:
        ...
        for chunk in texts:
            documents.append(Document(page_content=chunk,
                                      metadata={"filename": filename, "owner": owner, "title": title,
                                                "category": category, "page_number": page_num}))
...
def get_all_filenames(username):
    if username == "admin":
        ...
        all_documents_cursor = collection.find({}, projection={"metadata.filename": True, "metadata.title": True,
                                                               "metadata.category": True})
    else:
        # Regular users have access only to their files
        all_documents_cursor = collection.find({"metadata.owner": username},
                                               projection={"metadata.filename": True, "metadata.title": True,
                                                           "metadata.category": True})
...
def handle_chat(question, selected_filename):
    ...
    with st.chat_message("assistant"):
        ...
        response_content = ans["text"]
        # print(response_content)
        json_string = response_content.strip().strip('```json').strip('```')
        ...
        # response_placeholder.markdown(data["answer"])
        # st.session_state.messages.append(AIMessage(content=data["answer"]))
...
with st.sidebar:
    st.image(
        "https://i0.wp.com/opensource.org/wp-content/uploads/2023/01/datastax-logo-square_transparent-background.png",
